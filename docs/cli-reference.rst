...
``[FILE]...``
    One or more paths to C/C++/Rust source files or directories.
...
``-r``, ``--recursive``
    Recursively process all C/C++/Rust source files (``.c``, ``.cpp``, ``.cc``,
    ``.cxx``, ``.rs``) in directories. Header files (``.h``, ``.hpp``, ``.hxx``)
    are skipped unless explicitly included via a filter. Generates ``report.txt``
...
``--compile-commands <FILE>``
    ...
    list. Mutually exclusive with positional ``[FILE]...``.
