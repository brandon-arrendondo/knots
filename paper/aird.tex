...
SLOC is computed exclusive of lines belonging to nested function
definitions.  In C++, methods of locally-defined structs and in Python,
nested \texttt{def} statements appear inside the outer function's byte
span but represent independent logical units.  Without exclusion, those
lines would be counted in both the outer function and in the nested
function when it is analyzed separately.  Each line is attributed to
exactly one function: the innermost function whose span contains it.
...
\subsubsection{Formula Evolution}

The AIRD formula reached its current form through four calibration
iterations against the corpus described in \cref{sec:calibration}.
\Cref{tab:formula-history} documents the changes; each iteration was
motivated by a concrete calibration failure.
...
\toprule
\textbf{Version} & \textbf{Change and Rationale} \\
\midrule
v1 & Initial hypothesis: cognitive/50$\times$35, sloc/100$\times$25,
     nesting/8$\times$15, test/40$\times$25, doc/10$\times$15. \\
v2 & Test ceiling 40$\to$20.  The maximum observed \texttt{test\_score}
     across the corpus was $\approx$18; the v1 ceiling structurally
     halved the term's contribution relative to intent. \\
v3 & Cognitive weight 35$\to$45, test weight 25$\to$15; cognitive
     ceiling 50$\to$75, sloc ceiling 100$\to$200.  Corpus percentile
     analysis showed the SLOC ceiling at 100 saturated at p95; the
     weight shift reduced \texttt{test\_score} inflation of
     undocumented tool entry points. \\
v4 & Cognitive weight 45$\to$55, sloc weight 25$\to$15.  Empirical
     experiment (\cref{sec:empirical}) confirmed cognitive complexity
     is the primary correlate of AI-rated difficulty; SLOC
     over-contributed for shallow entry points. \\
v1.5.0 & Renamed AIM $\to$ AIRD; formula unchanged.  A second metric
          (AICP) was introduced to capture context-loading cost
          separately from reasoning cost. \\
\bottomrule
\end{tabular}
\caption{AIRD formula evolution.  Each change was driven by a concrete
calibration failure against the 32,205-function corpus.}
\label{tab:formula-history}
\end{table}
...
20 external calls was consistent across all 6 corpora (range 15--20
across lua, libcrc, mosquitto, hostap, sqlite, and curl), establishing
it as a stable cross-codebase upper bound (\cref{sec:calibration}).

Adding \texttt{external\_calls} to the AIRD formula was considered but
rejected.  In the corpus, hostapd's \texttt{hostapd\_config\_read\_eap\_user}
has slightly more external calls (17) than SQLite's
\texttt{whereLoopAddBtreeIndex} (12), yet the empirical experiment rated
\texttt{whereLoopAddBtreeIndex} substantially harder to reason about.
Every SLOC-to-external-call weight transfer that closed one gap widened
the other.  External call breadth predicts context-loading cost, not
reasoning cost; it belongs in a separate metric.  Mean external calls by
AIRD band across the corpus confirms the metric's orthogonality:
$2.74$ (low AIRD) $\to$ $8.69$ (mid) $\to$ $17.40$ (high) --- monotonically
increasing and thus informative, but not directly correlated with
reasoning difficulty within the high band.
...
\midrule
mosquitto & 2{,}559  & MQTT broker \\
SQLite    & 9{,}491  & Embedded database \\
curl      & 5{,}474  & HTTP/transfer library \\
hostap    & 13{,}343 & WPA supplicant/AP \\
Lua       & 1{,}304  & Scripting runtime \\
libcrc    & 34       & CRC algorithms \\
\midrule
\textbf{Total} & \textbf{32{,}205} & \\
\bottomrule
\end{tabular}
\caption{AIRD/AICP calibration corpus (knots v1.5.0 function counts).}
\label{tab:corpus}
\end{table}
...
\Cref{tab:aird-dist} shows the AIRD score distribution across each
corpus.  The distribution is heavily right-skewed in all six projects:
63--88\% of functions score $\leq 10$, and the high-AIRD tail
($\geq 76$) accounts for 0--2\% of functions.  This shape is consistent
with software complexity distributions generally: most functions are
simple; a small fraction accumulate disproportionate complexity.
...
\toprule
\textbf{Corpus} & \textbf{Fns} & \textbf{Avg} & \textbf{Max}
  & \textbf{0--10} & \textbf{11--25} & \textbf{26--50}
  & \textbf{$\geq$51} \\
\midrule
lua       & 1{,}304  &  5.0 &  87 & 87\% & 11\% & 2\% & 0\% \\
libcrc    &     34   &  8.2 &  66 & 88\% &  6\% & 0\% & 6\% \\
mosquitto & 2{,}559  & 10.1 &  97 & 71\% & 20\% & 6\% & 3\% \\
hostap    & 13{,}343 & 10.9 &  95 & 67\% & 23\% & 8\% & 3\% \\
sqlite    &  9{,}491 & 10.2 &  99 & 72\% & 17\% & 7\% & 5\% \\
curl      &  5{,}474 & 12.4 &  95 & 63\% & 23\% & 10\% & 4\% \\
\bottomrule
\end{tabular}
\caption{AIRD distribution across the calibration corpus (knots v1.5.0).
The $\geq$51 column combines the 51--75 and 76--100 bands; the high-AIRD
tail ($\geq$76) is 0--2\% in all corpora.}
\label{tab:aird-dist}
\end{table}
...
\subsection{Distribution Stability: v1.12.0 Re-evaluation}
\label{sec:reeval}

To detect formula or metric drift, we re-ran all six corpora against
knots v1.12.0 and compared results to the v1.5.0 baseline.
\Cref{tab:reeval} reports the comparison.
...
\toprule
\textbf{Corpus} & \multicolumn{2}{c}{\textbf{Functions}}
  & \multicolumn{2}{c}{\textbf{Avg AIRD}} & \textbf{Drift} \\
  & v1.5.0 & v1.12.0 & v1.5.0 & v1.12.0 & \\
\midrule
lua       &  1{,}304 &  1{,}880 & 5.0  & 6.1  & $+$1.1 \\
libcrc    &     34   &     34   & 8.2  & 9.9  & $+$1.7 \\
mosquitto &  2{,}559 &  5{,}549 & 10.1 & 9.6  & $-$0.5 \\
hostap    & 13{,}343 & 20{,}160 & 10.9 & 12.3 & $+$1.4 \\
sqlite    &  9{,}491 & 11{,}351 & 10.2 & 11.0 & $+$0.8 \\
curl      &  5{,}474 &  5{,}836 & 12.4 & 14.2 & $+$1.8 \\
\bottomrule
\end{tabular}
\caption{AIRD distribution stability: v1.5.0 baseline vs.\ v1.12.0
re-evaluation.  All per-corpus average AIRD values are within 2 points;
the corpus ordering (curl hardest, lua easiest) is preserved.}
\label{tab:reeval}
\end{table}

The function count increases are explained by new language support
added after v1.5.0: mosquitto ($+$117\%) and hostap ($+$51\%) contain
substantial C++ code now parsed by the C++ grammar; Lua ($+$44\%)
files previously fell through to a C fallback and are now parsed
with the correct Lua grammar.  The distribution \emph{shape} is
preserved across all corpora: the 0--10 bucket remains dominant
(59--85\%) and the high-AIRD tail ($\geq$76) remains 0--1\%.

Key validation functions remained stable across versions:
\texttt{whereLoopAddBtreeIndex} (AIRD 90$\to$93),
\texttt{hostapd\_config\_read\_eap\_user} (93$\to$95),
\texttt{mosquitto\_validate\_utf8} (57$\to$59),
\texttt{luaV\_execute} (87$\to$89 after a SLOC regression was corrected;
see \cref{sec:vmcase}), and \texttt{sqlite3VdbeExec} (90$\to$91).
Score shifts of 2--3 points reflect corpus updates (newer upstream
snapshots), not formula instability.

\subsection{C Macro Pattern: SLOC Deflation in vmcase Blocks}
\label{sec:vmcase}

During the v1.12.0 re-evaluation, \texttt{luaV\_execute} in
\texttt{lvm.c} was found to report SLOC~34 rather than the expected
$\approx$750.  The root cause is a C macro pattern that tree-sitter-c
misparses as nested function definitions.

Lua's VM interpreter dispatches opcodes via:
...
tree-sitter-c parses \texttt{vmcase(OP\_MOVE) \{ \ldots \}} as a
\texttt{function\_definition} node because the construct is
syntactically identical to a K\&R-style function definition.  The
\texttt{nested\_fn\_sloc} subtraction --- added to prevent double-counting
Python/Rust/JS nested function bodies --- then subtracted each
\texttt{vmcase} block from the outer function, collapsing SLOC from 751
to 34 and dropping AIRD from 87 to 76.

The fix distinguishes genuine nested function definitions (whose
declarator is a plain \texttt{identifier}) from macro call patterns
(whose declarator is a \texttt{parenthesized\_declarator}, i.e., the
arguments are a parenthesized expression).  Filtering on this criterion
in \texttt{accumulate\_nested\_sloc} restored \texttt{luaV\_execute}'s
SLOC to 751 and AIRD to 89.  No other function in the six corpora was
...
\subsection{Normalization Cap Signal Loss and Raw AIRD}
...
\textbf{Raw AIRD.}  We address this with a two-pass approach: the CI
gate continues to use the capped formula (\cref{eq:aird}), while a
separate \emph{raw AIRD} lifts the \(\min(\cdot, 1)\) constraints on the
cognitive and SLOC terms:

\begin{align}
\text{AIRD}_{\text{raw}} = &\;\left(\frac{C_c}{75} \times 55\right)
                           + \left(\frac{S}{200} \times 15\right)
                           + \ldots
\label{eq:aird-raw}
\end{align}

where nesting, test score, documentation, and coupling terms retain their
caps (they rarely exceed their normalization denominators in practice).
$\text{AIRD}_{\text{raw}}$ can exceed 100 and decreases monotonically as
cognitive complexity and SLOC are reduced --- even while both remain above
their normalization ceilings, where the capped gate score is pinned.

For example, a function with cognitive complexity 750 and SLOC 254 has a
capped AIRD of 87 (at the gate ceiling) and a raw AIRD of 586.  After an
extraction that brings cognitive to 400 and SLOC to 150, the capped AIRD
is unchanged (still 87 --- both inputs are above their caps), but the raw
AIRD falls to approximately 320, providing a concrete measure of progress.

Knots emits $\text{AIRD}_{\text{raw}}$ in the violation output whenever
cognitive or SLOC is capped (\cref{sec:implementation}).  It is
informational only and is not compared against thresholds.
...
src/lexer.rs:45:next_token - AIRD 94 > 85
    ...
    raw AIRD (uncapped): 147
    Tip: cognitive and sloc are both capped --
    incremental extractions won't move the needle
    until you break through at least one cap (raw
    AIRD: 147 -- tracks your true progress while
    the gate score is pinned). Push for a larger
    extraction that drops cognitive below 75 or
    sloc below 200.
\end{lstlisting}

The per-term breakdown with \texttt{[capped]} labels was motivated by
the dual-cap stagnation phenomenon (\cref{sec:plateau}).  The raw AIRD
line (\cref{sec:signal-loss}) appears whenever cognitive or SLOC is
capped, giving developers a directional progress signal while the gate
score is pinned.  The contextual tip is selected based on the function's
current state:
\begin{itemize}
    \item Both caps active: dual-cap stagnation tip (includes raw AIRD value).
