...
eslint, and mypy on legacy code. It complements the
[`--include`/`--exclude` filters](FILTERS.md): filters decide *what gets
analyzed*; the baseline decides *what counts as a failure*.
...
| Situation                                              | Result     |
|--------------------------------------------------------|------------|
| Function absent from baseline (new code)               | **fail**   |
| Metric value **>** its baselined value (got worse)     | **fail**   |
| Metric value **≤** its baselined value (same / better) | tolerated  |
| Function under all thresholds                          | tolerated  |

A function that is still over threshold but *better* than its baseline passes —
so partial progress is never punished. Only `--baseline` (not `--write-baseline`)
changes gating; `--write-baseline` just (re)writes the file and exits 0.
